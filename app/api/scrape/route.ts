  ...
  try {
    ...
    const extractStructuredData = () => {
      ...
      const isContentRich = (element: Element): boolean => {
        ...
        const skipPatterns = [
          'nav', 'navigation', 'menu', 'footer', 'header', 'banner', 
          'cookie', 'popup', 'modal', 'sidebar', 'widget', 'ad-', 
        ...
        const contentPatterns = [
          'content', 'article', 'post', 'main', 'body', 'text', 
          'story', 'entry', 'blog', 'news', 'description', 'info'
        ];
        ...
        const contentElements = element.querySelectorAll('p, h1, h2, h3, h4, h5, h6, ul, ol, blockquote');
      ...
      const processHeadings = () => {
        ...
        headings.forEach((heading, index) => {
          ...
          while (currentNode && !processedElements.has(currentNode)) {
            ...
            else if (nodeName === 'a' && !currentNode.parentElement?.closest('li')) {
      ...
      const processRemainingContent = () => {
        ...
        const contentSections = Array.from(document.querySelectorAll('article, section, main, [role="main"], [class*="content"], [id*="content"]'))
          .filter(section => isContentRich(section) && !processedElements.has(section));
        ...
        if (contentSections.length > 0) {
          contentSections.forEach((section) => {
            ...
            let sectionTitle = '';
            const potentialTitle = section.querySelector('h1, h2, h3, h4, h5, h6');
            if (potentialTitle) {
            ...
            if (!sectionTitle) {
              const id = section.id;
              if (id && !id.match(/^\d+$/)) {
                sectionTitle = id.replace(/[_-]/g, ' ').replace(/([A-Z])/g, ' $1').trim();
            ...
            lists.forEach(list => {
              ...
              if (listItems.length > 0) {
                result.content[sectionKey].items.push({
                  type: 'list',
                  listType: list.nodeName.toLowerCase() === 'ul' ? 'unordered' : 'ordered',
                  items: listItems
                });
              }
            });
...
async function optimizeWithMistral(data: any) {
  try {
    ...
    if (!MISTRAL_API_KEY) {
      console.warn('MISTRAL_API_KEY not found in environment variables. Returning unoptimized data.');
      return data;
    }
    ...
    try {
      const optimizedJson = JSON.parse(result.choices[0].message.content.trim());
